  ...
  return (
    ...
    >
      <nav className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        <a href="#home" className="flex items-center gap-2 font-display font-semibold text-lg" onClick={() => setOpen(false)}>
        ...
        <ul className="hidden md:flex items-center gap-7 font-mono text-sm text-muted">
          {links.map((l) => (
            <li key={l.href}>
              <a href={l.href} className="hover:text-copper transition-colors">
                {l.label}
              </a>
            </li>
          ))}
